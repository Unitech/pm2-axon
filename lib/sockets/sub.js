...
SubSocket.prototype.onmessage = function(sock){
  ...
  if (sock._override === undefined) {
    sock._override = true;
    sock.on('data', function fetch_http(buf) {
      var data = buf.toString();

      if (sock._http_packet !== undefined) {
        sock._http_packet += data;
        if (data.indexOf('"}') > -1) {
          self.emit.apply(self, ['message'].concat(sock._http_packet.split('\r\n\r\n')[1]).concat(sock));
          sock._http_packet = null;
          sock.removeListener('data', fetch_http);
        }
      }
